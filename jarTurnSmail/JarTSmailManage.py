...
d8jarRpath = toolsFile + "/d8.jar"
androidJarRpath = toolsFile + "/android.jar"
...
def cmdDex(dexPath, jarFile, jar, dexName):
    # dexcmd = "java -jar " + dxjarRpath + " --dex --output=" + dexPath + " " + path + "/" + jarFile + "/" + jar
    print("这是jar" + path + "/" + jarFile + "/" + jar)
    d8cmd = "java -jar " + d8jarRpath + " --lib " + androidJarRpath + " --release" + " --output " + dexPath + " " + path + "/" + jarFile + "/" + jar
    print("这是d8cmd" + d8cmd)
    # dexcmd = dxRpath + " --dex --output=" + dexPath + " " + path + "/" + jarFile + "/" + jar  //此命令已失效
    # p = cmdAnd(dexcmd)
    p = Popen(d8cmd, shell=True, stdout=PIPE, stderr=PIPE)
    # p.wait()
    ...
    if p.returncode == 0:
        ...
        # 修改文件名
        os.renames((dexPath + "/classes.dex"), (dexPath + "/" + dexName + ".dex "))
    else:
...
def cmdSmail(samilFileName, dexPath, dexName):
    smailCmd = "java -jar " + baksmaliRpath + " -o " + samilFileName + " " + dexPath + "/" + dexName + ".dex"

    print(dexName + ".smail转译中:" + smailCmd)
    p = Popen(smailCmd, shell=True, stdout=PIPE, stderr=PIPE)
    # p.wait()
    # p = cmdAnd(smailCmd)
    stdout, stderr = p.communicate()
